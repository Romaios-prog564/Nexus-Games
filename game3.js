...
class Target {
  ...
  reset() {
    ...
    this.r = this.baseR;
    this.alive = true;
    this.life = 2.5 - this.depth * 1.0;
    this.born = performance.now() / 1000;
    this.wobble = Math.random() * Math.PI * 2;
    this.wobbleSpeed = 1 + Math.random();
    // 10% chance golden target = triple points
    this.isGolden = Math.random() < 0.10;
    this.points = this.isGolden
      ? Math.round((10 + (1 - this.depth) * 20) * 3)
      : Math.round(10 + (1 - this.depth) * 20);
    this.color = this.isGolden ? '#ffdd00' : `hsl(${Math.random() * 60 + 330}, 100%, 60%)`;
...
function draw(now) {
  ...
  sorted.forEach(t => {
    ...
    if (t.isGolden) {
      ...
      ctx.shadowBlur = 35;
      for (let s = 0; s < 8; s++) {
        const a = (s / 8) * Math.PI * 2 + Date.now() / 400;
        ctx.fillStyle = 'rgba(255,220,0,0.8)';
        ctx.beginPath();
        ctx.arc(x + Math.cos(a) * (r + 8), y + Math.sin(a) * (r + 8), 2.5, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    // Target rings
    ctx.shadowColor = t.color;
    ctx.shadowBlur = t.isGolden ? 40 : 25;

    // Outer ring
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.strokeStyle = t.color;
    ctx.lineWidth = t.isGolden ? 3 : 2;
    ctx.stroke();

    // Middle ring
    ctx.beginPath();
    ctx.arc(x, y, r * 0.65, 0, Math.PI * 2);
    ctx.strokeStyle = t.isGolden ? 'rgba(255,255,200,0.8)' : 'rgba(255,255,255,0.6)';
    ctx.lineWidth = 1.5;
    ctx.stroke();
