...
let obstacles = [], particles_game = [], gems = [];
...
function startGame() {
  ...
  playerLane = 1; obstacles = []; particles_game = []; gems = [];
...
function loop() {
  ...
  frameCount++;
  speed = Math.min(4 + frameCount * 0.0015, 16);
  score = Math.floor(frameCount / 6);
  scoreDisplay.textContent = score;

  // Spawn obstacles
  if (frameCount - lastObstacleFrame > Math.max(40, 100 - frameCount * 0.04)) {
  ...
  // Spawn gem every 100 score points
  if (score > 0 && score % 100 === 0 && gems.length === 0 && frameCount % 2 === 0) {
    const lane = Math.floor(Math.random() * LANE_COUNT);
    gems.push({ lane, y: -30, x: getLaneX(lane), born: frameCount });
  }
  // Move gems
  gems.forEach(g => { g.y += speed * 0.8; });
  // Collect gem
  gems = gems.filter(g => {
    if (Math.abs(g.x - getLaneX(playerLane)) < 20 && Math.abs(g.y - playerY) < 20) {
      score += 25; scoreDisplay.textContent = score;
      spawnParticle(g.x, g.y, '#ffdd00');
      return false;
    }
    return g.y < H + 40;
  });
